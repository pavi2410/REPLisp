...
void lval_print_str_unescaped(lval *v) {
    char *unescaped = malloc(strlen(v->str) + 1);
    strcpy(unescaped, v->str);
...
lval *builtin_print(lenv *e, lval *a) {
    ...
    for (int i = 0; i < a->count; i++) {
        if (a->cell[i]->type == LVAL_STR) {
            lval_print_str_unescaped(a->cell[i]);
        } else {
            lval_print(a->cell[i]);
        }
        putchar(' ');
...
void eval_load_file(lenv *e, char *filename) {

    if (!str_endswith(filename, ".rep")) {
        return;
    }
