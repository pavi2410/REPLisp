...
fn builtin_add(args: &[Value]) -> Result<Value, EvalError> {
    ...
    for arg in args {
        match arg {
            ...
            _ => return Err(EvalError::TypeError("+ requires numbers".to_string(), Span::point(0))),
        }
    }
    Ok(Value::Number(sum))
}
...
fn builtin_subtract(args: &[Value]) -> Result<Value, EvalError> {
    if args.is_empty() {
        return Err(EvalError::ArityError("- requires at least 1 argument".to_string(), Span::point(0)));
    }
    ...
    match &args[0] {
        ...
        _ => Err(EvalError::TypeError("- requires numbers".to_string(), Span::point(0))),
    }
}
...
fn builtin_divide(args: &[Value]) -> Result<Value, EvalError> {
    ...
    match &args[0] {
        Value::Number(first) => {
            if args.len() == 1 {
                if *first == 0.0 {
                    return Err(EvalError::DivisionByZero(Span::point(0)));
                }
...
fn builtin_equal(args: &[Value]) -> Result<Value, EvalError> {
    if args.len() != 2 {
        return Err(EvalError::ArityError("= requires exactly 2 arguments".to_string(), Span::point(0)));
    }
...
fn builtin_min(args: &[Value]) -> Result<Value, EvalError> {
    if args.is_empty() {
        return Err(EvalError::ArityError("min requires at least 1 argument".to_string(), Span::point(0)));
    }
    
    let mut min_val = match &args[0] {
        Value::Number(n) => *n,
        _ => return Err(EvalError::TypeError("min requires numbers".to_string(), Span::point(0))),
    };
...
fn builtin_max(args: &[Value]) -> Result<Value, EvalError> {
    if args.is_empty() {
        return Err(EvalError::ArityError("max requires at least 1 argument".to_string(), Span::point(0)));
    }
    
    let mut max_val = match &args[0] {
        Value::Number(n) => *n,
        _ => return Err(EvalError::TypeError("max requires numbers".to_string(), Span::point(0))),
    };
...
fn builtin_abs(args: &[Value]) -> Result<Value, EvalError> {
    if args.len() != 1 {
        return Err(EvalError::ArityError("abs requires exactly 1 argument".to_string(), Span::point(0)));
    }
    
    match &args[0] {
        Value::Number(n) => Ok(Value::Number(n.abs())),
        _ => Err(EvalError::TypeError("abs requires a number".to_string(), Span::point(0))),
    }
}

fn builtin_mod(args: &[Value]) -> Result<Value, EvalError> {
    if args.len() != 2 {
        return Err(EvalError::ArityError("mod requires exactly 2 arguments".to_string(), Span::point(0)));
    }
    ...
    match (&args[0], &args[1]) {
        ...
        _ => Err(EvalError::TypeError("mod requires numbers".to_string(), Span::point(0))),
    }
}